...
class HBSinglePlayScene: HBPlayScene {
    ...
    override func didMove(to view: SKView) {
        ...
        self.movableAreaNode = SKSpriteNode(color: .blue, size: CGSize(width: self.frame.width, height: CGFloat(Paddle.PADDLE_RADIUS * 3)))
        ...
        let earth = SKSpriteNode(texture: self.earthTexture)
        earth.position = CGPoint(x: self.frame.midX, y: self.frame.minY)
    ...
    func addEnemy() {
        ...
        for i in 0..<numOfEnemies {
            ...
            let enemy : SKSpriteNode = self.newEnemy(type: enemyType)
            self.addChild(enemy)
        }
    }
    ...
    func addPaddle() {
        ...
        let paddle : SKSpriteNode = SKSpriteNode(texture: SKTexture(image: UIImage(named: "spaceCat.png")!))
    ...
    func shootBall() {
        ...
        self.ballNodes().forEach {
            $0.physicsBody?.velocity = CGVector(dx: velocityX + CGFloat(self.stage), dy: velocityY)
        }
    }
    ...
    func stopBalls() {
        self.ballNodes().forEach {
            $0.physicsBody?.velocity = CGVector(dx: 0.0, dy: 0.0)
    ...
    func compensateBallAngle(ballPhysicsBody : SKPhysicsBody) {
        ...
        if (CGFloat(abs(angle)) < CGFloat(Ball.BALL_ANGLE_MIN) || CGFloat(abs(angle)) > CGFloat(Ball.BALL_ANGLE_MAX)) {
            ...
            // let newAngle = newRad / CGFloat(Float.pi * 2) * 360.0
        }
    }
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        if (self.gameState != GAME_STATE.NORMAL) { return }
        self.removeTouchAreaAndText()
        if (!self.ballNodes().isEmpty && !self.isBallMoving()) {
            if let ball = self.ballNodes().first {
    ...
    func gameOver() {
        self.removeTouchAreaAndText()
        self.bestScoreManager.updateBestScoreForStage(stage: GAME_STAGE.NORMAL, score: self.score)
